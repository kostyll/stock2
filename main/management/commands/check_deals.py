...
def remove_orders(Date, Cancel):
    processed_orders = {}
    man2process = {}
    for item in Trans.objects.filter(pub_date__gte=Date).filter(
                                    Q(status='deposit') | Q(status="deal") | Q(status="comission") | Q(
                    status='order_cancel')):
    ...
    for trans_list in man2process.keys():
        ...
        for item in man2process[trans_list] + List:
            if not process_transes.has_key(item.id):
                print "%i %i to %i %s %s %s %s" % (
                item.id, item.user1_id, item.user2_id, item.pub_date, item.status, item.amnt, str(item.currency) )
                process_transes[item.id] = True


def search_trans_before(order_id, Date):
    List = list(Trans.objects.filter(pub_date__lt=Date, status='deal').filter(
        Q(out_order_id=str(order_id)) | Q(order_id=order_id)))
    for item in List:
