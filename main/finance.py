...
def confirm_p2p_withdraw_email_common(Post, Key):
        return _(u"На BITCOIN TRADE COMPANY вами заказана  операция вывода инструментов на Карту\n\n\
Карта: {card}\n\
Имя держателя: {name}\n\
Сумма: {amnt}\n\
Валюта: {currency}\n\n\n\
...
def confirm_crypto_withdraw_email(Post, Key):
              return _(u"Для подтверждение операции вывода инструментов на кошелек криптовалюты\n\n\
Кошелек: {account}\n\
Сумма: {amnt}\n\
Валюта: {currency}\n\n\n\
перейдите по ссылке {ref}\n\n\n\
...
def home(Req):
       if not Req.user.is_authenticated():
          ...
          BalancesOrders = OrdersMem.objects.filter(user = Req.user.id,
                                                    status = "processing").values('currency1').annotate(balances = Sum('sum1'))
          OrdersBalances = {}
...
def generate_description(DictAccounts, item):
        ...
        if item.status == 'order_cancel' :
                return _(u"Canceletion of order #{order_id}").format(order_id=item.order_id) 
        if item.status == 'bonus' :
                return _(u"Partnership reward")

        if item.status == 'deal_return' :
                return _(u"Return unused funds according with order  #{order_id}").format( order_id=item.order_id)  

        
        if item.status == 'deposit' :
                return _(u"Deposit funds according with order #{order_id}").format(order_id=item.order_id)  
        
        if item.status == 'comission' :
                return _(u" Commision from the deal in order #{order_id}").format(order_id=item.order_id) 
        if item.status == 'deal' :
                return _(u" Buying   according with the order #{order_id}").format(order_id=item.order_id) 
...
def trans(Req):
             ...
             Query = "SELECT * FROM main_trans WHERE 1 \
                               ...
                               AND ( user1_id IN (%s) OR user2_id IN  (%s) ) ORDER BY id DESC  " % ( AccountsStr, AccountsStr)
