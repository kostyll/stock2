...
from  main.http_common  import denied, http_tmpl_context, json_true, json_denied, http_json, caching, my_cached_paging, tornadocache_delete
import  main.models 
...
#send_mail(_(u'Обновление пароля на бирже ' + settings.BASE_HOST),
...
def msgs_out(Req):
          if not Req.user.is_authenticated():
             ...
             Dict["msg_in_count"] = main.models.Msg.objects.filter(user_to = Req.user, user_hide_to = "false" ).exclude(user_from_id = 1).count()
...
" % ( str(P2P.amnt), str(P2P.currency), P2P.CardNumber, P2P.CardName, user.username )    
...
\n\n\n\
С уважением служба поддержки BITCOIN TRADE COMPANY\n\
" % (str(Crypto.amnt), str(Crypto.currency), Crypto.account, user.username )   
...
\n\n\n\
С уважением служба поддержки BITCOIN TRADE COMPANY\n\
" % (str(DebP24.amnt), str(DebP24.currency.title), user.username )
...
\n\n\n\
С уважением служба поддержки BITCOIN TRADE COMPANY\n\
" % (str(DebCredLiqPay.amnt), str(DebCredLiqPay.currency.title), DebCredLiqPay.phone, user.username )
...
\n\
С уважением служба поддержки BITCOIN TRADE COMPANY\n\
" % (settings.BASE_URL + "pin_image_page/" + str(Key)  )
...
Пользователь : %s\n\
С уважением служба поддержки BITCOIN TRADE COMPANY\n\
" % (Req.META['REMOTE_ADDR'], user.username)
        else:
...
\n\n\n\
С уважением служба поддержки BITCOIN TRADE COMPANY\n\
" % ( str(P2P.amnt), str(P2P.currency), P2P.CardNumber, P2P.CardName, P2P.user.username, State )  
