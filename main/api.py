...
from main.http_common import caching, cached_json_object, my_cache, status_false, json_auth_required, auth_required, \
    check_api_sign
from main.http_common import format_numbers10, format_numbers_strong, format_numbers, format_numbers4, json_false500, \
    json_true
import logging
...
def process_auto(Req, Res, TradePair):
    ...
    for i in Res["users_bothered"]:
        CachedKey1 = 'client_orders_' + str(i.id) + "_" + Type
...
def __inner_remove_order(Order, User):
    ...
    Order2Remove.save()
    OrderMyIsam = Orders.objects.get(id=int(Order))
    Title = OrderMyIsam.trade_pair.url_title
    LOCK = "trades" + Title
    TradeLock = my_lock(LOCK)

    try:

        OrderMyIsam.sum1 = Order2Remove.sum1
        OrderMyIsam.sum2 = Order2Remove.sum2
        OrderMyIsam.status = "canceled"
...
def buy(Req, Trade_pair):
    ...
    try:
        ...
        ResAuto = make_auto_trade(MemOrder, TradePair, order.price, CurrencyBase, Amnt1, CurrencyOn, Amnt2)
